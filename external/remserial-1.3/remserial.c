...
int main(int argc, char *argv[])
{
	...
	while ( (c=getopt(argc,argv,"dl:p:r:s:wx:")) != EOF )
		switch (c) {
		case 'd':
			isdaemon = 1;
			break;
	...
	*/
	...
	while (1) {
		...
		if ( !machinename && FD_ISSET(sockfd,&fdsreaduse) ) {
			...
			remotefd=accept(sockfd,(struct sockaddr*)(&remoteaddr),
				&remoteaddrlen);
			if ( remotefd == -1 )
		...
		if ( FD_ISSET(devfd,&fdsreaduse) ) {
			...
			if ( devbytes <= 0 ) {
				...
				write(remotefd,devbuf,devbytes);
		}

		/* Data to read from the remote system */
		if ( remotefd != -1 && FD_ISSET(remotefd,&fdsreaduse) ) {
			devbytes = read(remotefd,devbuf,512);
